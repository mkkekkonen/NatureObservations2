...
const migration: IMigration = {
  ...
  forwards: async (adapter: AbstractDbAdapter) => {
    ...
    const sql = [];
    sql.push('CREATE TABLE IF NOT EXISTS observation_new (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, description TEXT, date TEXT, type TEXT NOT NULL, mapLocationId INTEGER, imgDataId INTEGER, '
    ...
    if (observations.length > 0) {
      sql.push('INSERT INTO observation_new (title, description, date, type, mapLocationId, imgDataId) VALUES ' + observationValues);
    }
    sql.push('DROP TABLE IF EXISTS observation');
    sql.push('ALTER TABLE observation_new RENAME TO observation');
